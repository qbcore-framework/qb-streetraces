...
RegisterNetEvent('qb-streetraces:RaceWon', function(RaceId)
    ...
    if Config.UseCharName then
        ...
        TriggerClientEvent('qb-streetraces:RaceDone', -1, RaceId, GetPlayerName(src)) -- Use Account name, acts as default
    end
end)
                ...
                if not Races[key].started then
                    for _, iden in pairs(Races[key].joined) do
                        local xdPlayer = QBCore.Functions.GetPlayer(iden)
                            ...
                            TriggerClientEvent('QBCore:Notify', xdPlayer.PlayerData.source, "The race has stopped, your $"..Races[key].amount.." got refunded.", 'error')
